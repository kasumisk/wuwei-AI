...
async function checkGoalTransition(userId: string): Promise<GoalTransitionSuggestion | null> {
  ...
  // fat_loss 达成
  if (profile.goal === GoalType.FAT_LOSS && profile.weightKg <= profile.targetWeightKg) {
    return {
  ...
  // 长期停滞（4 周无变化）
  if (inferred.goalProgress.trend === 'behind' && inferred.goalProgress.estimatedWeeksLeft > 20) {
    return {
...
async function onProfileUpdate(userId: string, oldProfile: UserProfile, newProfile: UserProfile) {
  ...
  const changed = criticalFields.filter(
    (f) => JSON.stringify(oldProfile[f]) !== JSON.stringify(newProfile[f])
  );
