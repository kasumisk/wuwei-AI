...
const GOAL_WEIGHTS: Record<GoalType, NutritionWeights> = {
  fat_loss: {
    ...
    foodQuality: 0.1, // 远离加工食品
    satiety: 0.1, // 吃饱防暴食
    stability: 0.05, // 坚持记录
  },
  muscle_gain: {
    proteinRatio: 0.3, // 蛋白质第一优先
    energy: 0.25, // 热量必须够
    macroBalance: 0.2, // 碳水供能、脂肪适宜
    foodQuality: 0.1, // 干净增肌
    satiety: 0.05, // 次要
    stability: 0.1, // 规律饮食很重要
  },
  health: {
    foodQuality: 0.3, // 食物天然度最高优先
    macroBalance: 0.2, // 营养均衡
    energy: 0.15, // 不超标即可
    satiety: 0.15, // 舒适不饿
    proteinRatio: 0.1, // 适量蛋白
    stability: 0.1, // 规律饮食
  },
  habit: {
 ...
 *   习惯：15%-30%（宽松）
 */
function calcProteinRatioScore(protein: number, calories: number, goal: GoalType): number {
 ...
 * 取两者的平均分
 */
function calcMacroScore(carbs: number, fat: number, calories: number): number {
  if (calories <= 0) return 80;
...
```typescript
function applyPenalties(score: number, input: NutritionInput): number {
  let penalized = score;
...
export function calculateNutritionScore(
  ...
  const proteinRatio = calcProteinRatioScore(input.protein, input.calories, goal);

  const macroBalance = calcMacroScore(input.carbs, input.fat, input.calories);
...
export class NutritionScoreService {
  ...
  private calcMacroScore(carbs: number, fat: number, calories: number): number {
    ...
    const carbRatio = (carbs * 4) / calories;
    const fatRatio = (fat * 9) / calories;
    return (this.rangeScore(carbRatio, 0.4, 0.55) + this.rangeScore(fatRatio, 0.2, 0.3)) / 2;
  }
  ...
  private generateHighlights(scores: NutritionScoreBreakdown, input: NutritionInput): string[] {
    const hl: string[] = [];
...
```typescript
// 评分覆盖 + 安全兜底
function resolveDecision(aiDecision: string, engineDecision: string): string {
