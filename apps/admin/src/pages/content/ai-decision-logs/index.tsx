...
const AiDecisionLogsPage: React.FC = () => {
  ...
  const columns: ProColumns<AiDecisionLogDto>[] = [
    ...
    },
    {
      ...
      dataIndex: 'riskLevel',
      width: 80,
      render: (_, r) => r.riskLevel ? <Tag color={riskMap[r.riskLevel] || 'default'}>{r.riskLevel}</Tag> : '-',
    },
    {
      title: '用户是否执行',
      ...
      width: 100,
      search: false,
      render: (_, r) => r.userFollowed === null ? '-' : r.userFollowed ? <Tag color="success">是</Tag> : <Tag color="error">否</Tag>,
    },
    { title: '用户反馈', dataIndex: 'userFeedback', width: 100, search: false, render: (v) => v || '-' },
    ...
    <>
      {stats && (
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col span={6}><Card><Statistic title="总决策数" value={stats.total} /></Card></Col>
          {stats.byDecision?.slice(0, 3).map((d: any) => (
            <Col span={6} key={d.decision}>
              <Card><Statistic title={d.decision || '未知'} value={d.count} /></Card>
            </Col>
          ))}
        </Row>
      )}
      ...
      >
        {currentLog && (
          <div>
            <h4>输入上下文</h4>
            <pre style={{ background: '#f5f5f5', padding: 12, borderRadius: 6, maxHeight: 200, overflow: 'auto' }}>
