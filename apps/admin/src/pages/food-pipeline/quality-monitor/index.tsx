...
const QualityMonitorPage: React.FC = () => {
  ...
  if (!report) {
    return <Card><Text type="secondary">暂无数据质量报告</Text></Card>;
  }
  ...
  // 计算综合得分
  const completenessScore = completeness ? Math.round(
    ((completeness.hasMacros + completeness.hasMicros + completeness.hasAllergens +
      completeness.hasImage + completeness.hasBarcode + completeness.hasMealTypes +
      completeness.hasCompatibility) / (total * 7)) * 100
  ) : 0;

  const qualityScore = quality ? Math.round(
  ...
  return (
    <div>
        ...
        <Col xs={12} sm={6}>
          <Card>
            <div style={{ textAlign: 'center' }}>
              <Text type="secondary" style={{ fontSize: 14 }}>完整度得分</Text>
              <Progress type="dashboard" percent={completenessScore} size={80} strokeColor={completenessScore > 70 ? '#52c41a' : completenessScore > 40 ? '#faad14' : '#ff4d4f'} />
            </div>
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card>
            <div style={{ textAlign: 'center' }}>
              <Text type="secondary" style={{ fontSize: 14 }}>质量得分</Text>
              <Progress type="dashboard" percent={qualityScore} size={80} strokeColor={qualityScore > 70 ? '#52c41a' : qualityScore > 40 ? '#faad14' : '#ff4d4f'} />
            </div>
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{ color: quality?.macroConsistencyPass && quality.macroConsistencyPass / total > 0.8 ? '#3f8600' : '#cf1322' }}
            />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={24} md={12}>
          <Card
            title={<Space><BarChartOutlined /> 字段完整度</Space>}
            extra={<Button size="small" icon={<SyncOutlined />} onClick={() => refetch()}>刷新</Button>}
          >
            {completeness && (
                ...
                {[
                  ...
                  return (
                    <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <Progress percent={pct} size="small" style={{ flex: 1 }} strokeColor={item.color} />
                      <Text type="secondary" style={{ width: 70, textAlign: 'right', flexShrink: 0 }}>{item.value}/{total}</Text>
                    </div>
                  );
                })}
              </Space>
        ...
        <Col xs={24} md={12}>
          <Card title={<Space><CheckCircleOutlined /> 质量指标</Space>}>
            <Descriptions column={1} size="small">
              <Descriptions.Item label="已验证食物">
                <Space>
                  <Tag color="green">{quality?.verifiedCount || 0}</Tag>
                  <Text type="secondary">({((quality?.verifiedCount || 0) / total * 100).toFixed(1)}%)</Text>
                </Space>
              </Descriptions.Item>
              <Descriptions.Item label="平均置信度">
                <Progress
                  ...
                  size="small"
                  status={quality?.avgConfidence && quality.avgConfidence > 0.7 ? 'success' : 'exception'}
                />
              </Descriptions.Item>
              <Descriptions.Item label="数据状态分布">
                <Space wrap>
                  {statusData.map(s => (
                    <Tag key={s.status} color={s.status === 'active' ? 'green' : s.status === 'draft' ? 'default' : 'orange'}>
                      {s.status}: {s.count}
                    </Tag>
                  ))}
                </Space>
              ...
              <Descriptions.Item label="待处理冲突">
                <Tag color={conflicts?.pending ? 'red' : 'green'} icon={conflicts?.pending ? <WarningOutlined /> : <CheckCircleOutlined />}>
          ...
          <Card title="分类分布">
            <Table
              ...
              columns={[
                ...
                { title: '占比', dataIndex: 'percent', key: 'percent', render: (v: string) => `${v}%` },
              ]}
              rowKey="category"
              pagination={false}
              size="small"
