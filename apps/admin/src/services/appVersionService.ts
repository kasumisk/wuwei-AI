...
export const appVersionQueryKeys = {
  ...
  appVersion: (id: string) => [...appVersionQueryKeys.appVersions, 'detail', id] as const,
...
export const appVersionApi = {
  getAppVersions: async (params?: GetAppVersionsQueryDto): Promise<AppVersionsListResponseDto> => {
    return await request.get<AppVersionsListResponseDto>(PATH.ADMIN.APP_VERSIONS, params);
  },

  getAppVersionById: async (id: string): Promise<AppVersionInfoDto> => {
    return await request.get<AppVersionInfoDto>(`${PATH.ADMIN.APP_VERSIONS}/${id}`);
  },

  createAppVersion: async (data: CreateAppVersionDto): Promise<AppVersionInfoDto> => {
    return await request.post<AppVersionInfoDto>(PATH.ADMIN.APP_VERSIONS, data);
  },

  updateAppVersion: async (id: string, data: UpdateAppVersionDto): Promise<AppVersionInfoDto> => {
    return await request.put<AppVersionInfoDto>(`${PATH.ADMIN.APP_VERSIONS}/${id}`, data);
  },
  ...
  publishAppVersion: async (
    ...
    return await request.post<AppVersionInfoDto>(`${PATH.ADMIN.APP_VERSIONS}/${id}/publish`, data);
  },

  archiveAppVersion: async (id: string): Promise<AppVersionInfoDto> => {
    return await request.post<AppVersionInfoDto>(`${PATH.ADMIN.APP_VERSIONS}/${id}/archive`);
  },

  getStats: async (): Promise<AppVersionStatsDto> => {
    return await request.get<AppVersionStatsDto>(`${PATH.ADMIN.APP_VERSIONS}/stats`);
  },

  // ---- 渠道包 API ----
  getPackages: async (versionId: string): Promise<AppVersionPackageDto[]> => {
    return await request.get<AppVersionPackageDto[]>(PATH.ADMIN.APP_VERSION_PACKAGES(versionId));
  },
  ...
  deletePackage: async (versionId: string, packageId: string): Promise<void> => {
    await request.delete(`${PATH.ADMIN.APP_VERSION_PACKAGES(versionId)}/${packageId}`);
  },
  ...
  getStoreDefaults: async (): Promise<StoreDefaultsDto> => {
    return await request.get<StoreDefaultsDto>(PATH.ADMIN.APP_VERSION_STORE_DEFAULTS);
  },
};

// ==================== React Query Hooks ====================

export const useAppVersions = (params?: GetAppVersionsQueryDto, options?: any) => {
  return useQuery({
...
export const useAppVersionStats = (
  options?: Omit<
    import('@tanstack/react-query').UseQueryOptions<AppVersionStatsDto>,
    'queryKey' | 'queryFn'
  >
) => {
...
export const usePublishAppVersion = (options?: any) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data?: { releaseDate?: string } }) =>
...
export const useCreatePackage = (versionId: string, options?: any) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: CreatePackageDto) => appVersionApi.createPackage(versionId, data),
    onSuccess: () => {
...
export const useDeletePackage = (versionId: string, options?: any) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (packageId: string) => appVersionApi.deletePackage(versionId, packageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: packageQueryKeys.packages(versionId) });
      queryClient.invalidateQueries({ queryKey: appVersionQueryKeys.appVersions });
    },
...
export const useTogglePackageEnabled = (versionId: string, options?: any) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (packageId: string) => appVersionApi.togglePackageEnabled(versionId, packageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: packageQueryKeys.packages(versionId) });
    },
    ...options,
