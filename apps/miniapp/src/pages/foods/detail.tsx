...
function FoodDetailPage() {
  ...
  useEffect(() => { if (name) loadDetail() }, [name])

  const loadDetail = async () => {
    setLoading(true)
    try {
      const res = await foodLibrary.getByName(name)
      setFood(res.food); setRelated(res.relatedFoods || [])
  ...
  const handleAdd = async () => {
    ...
    } catch (err: any) { Taro.showToast({ title: err?.message || '添加失败', icon: 'none' }) }
    finally { setAdding(false) }
  }

  if (loading) return <View className='min-h-screen bg-gray-50 flex items-center justify-center'><Text className='text-sm text-gray-400'>加载中...</Text></View>
  if (!food) return <View className='min-h-screen bg-gray-50 flex items-center justify-center'><Text className='text-sm text-gray-400'>未找到该食物</Text></View>
      ...
      <View className='bg-white rounded-2xl p-5 mb-4'>
        ...
        <View className='flex justify-around'>
          <View className='text-center'>
            <Text className='block text-base font-semibold'>{food.proteinPer100g != null ? Math.round(food.proteinPer100g * factor) : '-'}g</Text>
            <Text className='block text-xs text-gray-400 mt-1'>蛋白质</Text>
          </View>
          <View className='text-center'>
            <Text className='block text-base font-semibold'>{food.fatPer100g != null ? Math.round(food.fatPer100g * factor) : '-'}g</Text>
            <Text className='block text-xs text-gray-400 mt-1'>脂肪</Text>
          </View>
          <View className='text-center'>
            <Text className='block text-base font-semibold'>{food.carbsPer100g != null ? Math.round(food.carbsPer100g * factor) : '-'}g</Text>
            <Text className='block text-xs text-gray-400 mt-1'>碳水</Text>
          </View>
          {food.fiberPer100g != null && (
            <View className='text-center'>
              <Text className='block text-base font-semibold'>{Math.round(food.fiberPer100g * factor)}g</Text>
        ...
        <View className='mt-5'>
          ...
          {related.map(r => (
            <View
              ...
              onClick={() => Taro.navigateTo({ url: '/pages/foods/detail?name=' + encodeURIComponent(r.name) })}
            >
