...
function CoachPage() {
  ...
  const handleSend = async (text?: string) => {
    ...
    scrollToBottom();
    try {
      coachService.sendMessageStream(
        msg,
        conversationId,
        (fullText) => {
          setMessages((prev) =>
  ...
  const handleSendFallback = async (msg: string, aId: string) => {
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === aId ? { ...m, content: res.message.content, streaming: false } : m
        )
      );
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === aId ? { ...m, content: '抱歉，请求失败，请重试', streaming: false } : m
        )
      );
    } finally {
      setSending(false);
  ...
  const handleShowHistory = async () => {
    try {
      const list = await coachService.getConversations();
  ...
  const handleLoadConversation = async (conv: CoachConversation) => {
    try {
      ...
      setMessages(
        res.items.map((m: CoachMessage) => ({ id: m.id, role: m.role, content: m.content }))
      );
    ...
    return (
      <View className="flex flex-col h-screen bg-gray-50">
        <View className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-100">
          <Text className="text-sm text-blue-500" onClick={() => setShowHistory(false)}>
            ← 返回
          </Text>
        ...
        <ScrollView scrollY className="flex-1 p-5">
          {conversations.length === 0 ? (
            <View className="py-20 text-center">
              <Text className="text-sm text-gray-400">暂无历史对话</Text>
            </View>
          ) : (
  ...
  return (
    <View className="flex flex-col h-screen bg-gray-50">
      <View className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-100">
        <Text className="text-sm text-blue-500" onClick={handleNewChat}>
          ＋ 新对话
        </Text>
        <Text className="text-base font-semibold">AI 营养教练</Text>
        <Text className="text-sm" onClick={handleShowHistory}>
          📋
        </Text>
      </View>
      ...
      <View className="coach-input-bar flex items-center gap-3 px-5 py-3 bg-white border-t border-gray-100">
        ...
        />
        <Button
          className="coach-send-btn shrink-0"
