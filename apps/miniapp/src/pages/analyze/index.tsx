...
function AnalyzePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await foodService.saveRecord({
        foods: editFoods, totalCalories, mealType,
        imageUrl: result.imageUrl, advice: result.advice, isHealthy: result.isHealthy, source: 'camera',
      })
      setStep('saved')
    } catch (err: any) {
  ...
  const handleReset = () => { setStep('upload'); setImagePath(''); setResult(null); setEditFoods([]) }
    ...
    return (
      <View className='min-h-screen bg-gray-50 p-5'>
        ...
        <View className='mb-5'>
          ...
          <View className='upload-area bg-white rounded-2xl overflow-hidden' onClick={handleChooseImage}>
            {imagePath ? (
  ...
  if (step === 'saved') {
    return (
      <View className='min-h-screen bg-gray-50 flex flex-col items-center justify-center px-8'>
        ...
        <Button className='analyze-btn mb-4' onClick={handleReset}>继续记录</Button>
        <Button className='analyze-btn-outline' onClick={() => Taro.switchTab({ url: '/pages/index/index' })}>返回首页</Button>
      </View>
    )
  }
  ...
  return (
    <View className='min-h-screen bg-gray-50 p-5'>
      {imagePath && <Image className='w-full rounded-2xl mb-4 result-img' src={imagePath} mode='aspectFill' />}
      ...
      <View className='bg-white rounded-2xl p-5 mb-4'>
        <View className='flex items-baseline gap-1'>
          ...
          {result?.isHealthy != null && (
            <View className={`ml-2 px-2 py-1 rounded-full text-xs ${result.isHealthy ? 'bg-green-50 text-green-500' : 'bg-yellow-50 text-yellow-500'}`}>
              <Text>{result.isHealthy ? '健康' : '偏高'}</Text>
            </View>
          )}
        </View>
        {result?.advice && <Text className='block text-sm text-gray-500 mt-3 leading-relaxed'>💡 {result.advice}</Text>}
      </View>

      <View className='mb-5'>
        <Text className='block text-base font-semibold mb-3'>识别到的食物</Text>
        {editFoods.map((food, idx) => (
          <View className='flex items-center justify-between bg-white rounded-xl p-4 mb-2' key={idx}>
            <View className='flex-1'>
              <Text className='block text-sm font-medium'>{food.name}</Text>
              {food.quantity && <Text className='block text-xs text-gray-400 mt-1'>{food.quantity}</Text>}
            </View>
            <View className='flex items-center gap-3'>
              <Text className='text-sm text-blue-500 font-medium'>{food.calories} 千卡</Text>
              <Text className='text-sm text-red-400 px-1' onClick={() => setEditFoods(prev => prev.filter((_, i) => i !== idx))}>✕</Text>
            </View>
          </View>
        ))}
      </View>
      ...
      <View className='flex gap-3 pb-8'>
        <Button className='analyze-btn-outline flex-1' onClick={handleReset}>重新上传</Button>
        <Button className='analyze-btn flex-1' loading={saving} onClick={handleSave}>确认保存</Button>
      </View>
    </View>
  )
}
