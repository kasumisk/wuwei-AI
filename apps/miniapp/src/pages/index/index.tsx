  ...
  return (
    <ScrollView className='idx-page' scrollY>
      ...
      <View className='idx-header'>
        <View className='idx-avatar' onClick={() => Taro.navigateTo({ url: '/pages/profile/index' })}>
        ...
        <View className='idx-status-top'>
          ...
          {score > 0 && (
            <View className='idx-score-badge' style={{ borderColor: scoreColor }}>
              <Text className='idx-score-num' style={{ color: scoreColor }}>{score}</Text>
              <Text className='idx-score-unit' style={{ color: scoreColor }}>分</Text>
              <Text className='idx-score-tag' style={{ color: scoreColor }}>{scoreLabel}</Text>
            </View>
          )}
        </View>
        ...
        {metrics.length > 0 && (
          <View className='idx-metrics'>
            <Text className='idx-metrics-title'>
              {goalType === 'fat_loss' ? '🔥 减脂' : goalType === 'muscle_gain' ? '💪 增肌' : '🧘 ' + (goalLabel[goalType] || '健康')}用户关注
            </Text>
            {metrics.map(m => (
              <View key={m.key} className='idx-metric-row'>
                ...
                <View className='idx-metric-right'>
                  <View className='idx-metric-bar-track'>
                    <View className='idx-metric-bar-fill' style={{ width: `${m.pct}%`, background: m.pct > 100 ? '#ef4444' : '#16a34a' }} />
                  </View>
                  <Text className='idx-metric-val'>{Math.round(m.val)}<Text className='idx-metric-unit'>/{Math.round(m.goal)}{m.unit}</Text></Text>
                </View>
              </View>
            ))}
          </View>
      ...
      <View className='idx-cta-row'>
        <View className='idx-cta-main' onClick={() => Taro.switchTab({ url: '/pages/analyze/index' })}>
          <Text className='idx-cta-icon'>📷</Text>
          <Text className='idx-cta-text'>拍照识别</Text>
        </View>
        <View className='idx-cta-sub' onClick={() => Taro.navigateTo({ url: '/pages/foods/index' })}>
          <Text className='idx-cta-icon'>🔍</Text>
            ...
            <>
              <View className='idx-tab-row'>
                {suggestion.scenarios.map((s, i) => (
                  <View key={i} className={`idx-tab ${activeScenario === i ? 'idx-tab--on' : ''}`}
      ...
      {records.length > 0 && (
        <View className='idx-card'>
          <Text className='idx-section-title' style={{ marginBottom: '12px' }}>今日饮食记录</Text>
          {records.map((r) => (
