...
function ProfilePage() {
  ...
  useDidShow(() => {
    foodService.getProfile().then(p => setProfile(p)).catch(() => {})
  })
    ...
    <View className='me-page'>
      ...
      <View className='me-header'>
        ...
        <View className='me-user-info'>
          ...
          {user?.phone && (
            <Text className='me-phone'>{user.phone.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')}</Text>
          )}
        </View>
      </View>
      ...
      {profile && (
        <View className='me-card me-card--summary'>
          <View className='me-summary-row'>
            ...
            <View className='me-summary-divider' />
            <View className='me-summary-item'>
              <Text className='me-summary-val'>{profile.goal ? goalLabelMap[profile.goal] : '--'}</Text>
              <Text className='me-summary-key'>目标</Text>
            </View>
            <View className='me-summary-divider' />
            <View className='me-summary-item'>
              <Text className='me-summary-val'>{profile.activityLevel ? activityLabelMap[profile.activityLevel] : '--'}</Text>
        ...
        <View className='me-menu-item' onClick={goEditProfile}>
          ...
          <View className='me-menu-right'>
            {!profile?.onboardingCompleted && (
              <Text className='me-menu-badge'>未完善</Text>
            )}
      ...
      <View className='me-footer'>
        <Button className='me-logout-btn' onClick={logout}>退出登录</Button>
      </View>
