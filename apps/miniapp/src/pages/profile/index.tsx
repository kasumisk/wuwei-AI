...
function ProfilePage() {
  ...
  const Chips = ({ field, opts }: { field: keyof UserProfile; opts: { key: string; label: string }[] }) => (
    <View className='pf-chips'>
      {opts.map(o => {
        const active = ((profile[field] as string[]) || []).includes(o.key)
        return (
          <View key={o.key} className={`pf-chip ${active ? 'pf-chip--on' : ''}`} onClick={() => toggleChip(field, o.key)}>
            <Text>{o.label}</Text>
          </View>
        )
      })}
    ...
    <ScrollView className='pf-page' scrollY>
      ...
      <View className='pf-body'>
        ...
        <View className='pf-card'>
          {/* 性别 */}
          <View className='pf-row-label'><Text className='pf-label'>性别</Text></View>
          <View className='pf-gender-row'>
            <View className={`pf-gender-btn ${profile.gender === 'male' ? 'pf-gender-btn--on' : ''}`}
              onClick={() => up('gender', 'male')}>
              <Text>男</Text>
            </View>
            <View className={`pf-gender-btn ${profile.gender === 'female' ? 'pf-gender-btn--on' : ''}`}
          ...
          {/* 出生年份 */}
          <View className='pf-label-row'><Text className='pf-label'>出生年份</Text></View>
          <Input
            className='pf-input'
            type='number'
            placeholder='例如 1995'
          ...
          <View className='pf-row-2col'>
            <View className='pf-col'>
              ...
              <Input className='pf-input' type='digit' placeholder='170'
            ...
            <View className='pf-col'>
              ...
              <Input className='pf-input' type='digit' placeholder='65'
          ...
          {/* 目标体重 */}
          <View className='pf-label-row'><Text className='pf-label'>目标体重 (kg)</Text></View>
          <Input className='pf-input' type='digit' placeholder='60'
        ...
        <View className='pf-card'>
          ...
          <View className='pf-row-chips'>
            <View className={`pf-num-btn ${profile.canCook ? 'pf-num-btn--on' : ''}`}
          ...
          <Text className='pf-sub-title'>饮食偏好 <Text className='pf-hint'>（可多选）</Text></Text>
          <Chips field='foodPreferences' opts={preferenceChips} />

          <Text className='pf-sub-title'>忌口 <Text className='pf-hint'>（可多选）</Text></Text>
        ...
        <View className='pf-card'>
          ...
          <Text className='pf-sub-title'>容易乱吃时段 <Text className='pf-hint'>（可多选）</Text></Text>
        ...
        <View className='pf-card'>
          ...
          <Input className='pf-input' type='number' placeholder='自动计算'
        ...
        <View className='pf-actions'>
          ...
          <Button className='pf-logout-btn' onClick={logout}>退出登录</Button>
        </View>
