  ...
  return (
    <View className="flex flex-col min-h-screen bg-gray-50">
      ...
      <ScrollView
        ...
        onScrollToLower={() => {
          if (records.length < total && !loading) loadRecords(page + 1);
        }}
      >
        {records.length === 0 && !loading ? (
          <View className="py-20 text-center">
            <Text className="text-sm text-gray-400">暂无记录</Text>
          </View>
        ) : (
          records.map((record) => (
            <View className="bg-white rounded-xl p-4 mb-3" key={record.id}>
              <View className="flex">
                {record.imageUrl && (
                  <Image
                    className="record-img shrink-0 rounded-lg mr-3"
                    src={record.imageUrl}
                    mode="aspectFill"
                  />
                )}
                <View className="flex-1 min-w-0">
                  <Text className="block text-sm font-medium truncate">
                    {record.foods?.map((f) => f.name).join('、') || '未知食物'}
                  </Text>
                  <Text className="block text-xs text-blue-500 mt-1">
                    {mealTypeMap[record.mealType]} · {record.totalCalories} 千卡
                  </Text>
                  <Text className="block text-xs text-gray-400 mt-1" style={{ fontSize: '20rpx' }}>
                    {new Date(record.recordedAt || record.createdAt).toLocaleString()}
                  </Text>
                </View>
              </View>
              {record.advice && (
                <Text className="block text-xs text-gray-500 mt-3 leading-relaxed">
                  💡 {record.advice}
                </Text>
              )}
              <View className="flex justify-end mt-2">
                <Text className="text-xs text-red-400 px-2" onClick={() => handleDelete(record.id)}>
                  删除
                </Text>
              </View>
            </View>
