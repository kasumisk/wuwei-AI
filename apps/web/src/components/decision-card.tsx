...
import type { AnalysisResult } from '@/lib/api/food';

const DECISION_CONFIG = {
  SAFE: { emoji: '🟢', label: '放心吃', bgClass: 'bg-green-50 border-green-200', textClass: 'text-green-800', badgeClass: 'bg-green-100 text-green-800' },
  OK: { emoji: '🟡', label: '注意份量', bgClass: 'bg-yellow-50 border-yellow-200', textClass: 'text-yellow-800', badgeClass: 'bg-yellow-100 text-yellow-800' },
  LIMIT: { emoji: '🟠', label: '建议少吃', bgClass: 'bg-orange-50 border-orange-200', textClass: 'text-orange-800', badgeClass: 'bg-orange-100 text-orange-800' },
  AVOID: { emoji: '🔴', label: '不建议', bgClass: 'bg-red-50 border-red-200', textClass: 'text-red-800', badgeClass: 'bg-red-100 text-red-800' },
} as const;
...
export function DecisionCard({ result }: DecisionCardProps) {
  ...
  return (
    <div className={`rounded-2xl border p-5 space-y-4 ${config.bgClass}`}>
      ...
      {/* 补救策略 */}
      {result.compensation && (result.compensation.diet || result.compensation.activity || result.compensation.nextMeal) && (
        <div className="bg-white/60 rounded-xl p-3">
