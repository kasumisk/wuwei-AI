...
export class TextFoodAnalysisService {
  ...
  private async matchFoodLibrary(
    ...
    try {
      ...
      // 模糊搜索：按 sim_score 降序排列，取最高分
      const results = (await this.foodLibraryService.search(
        foodName,
        5,
      )) as any[];
      if (results.length > 0) {
        // 结果已按 sim_score DESC 排序，取第一个（最高相似度）
        const best = results[0];
        const simScore = Number(best.sim_score) || 0;

        // sim_score >= 0.3 或名称包含关系才认为匹配成功
        if (
          simScore >= 0.3 ||
          best.name.includes(foodName) ||
          foodName.includes(best.name) ||
          (best.aliases &&
            best.aliases.split(',').some((a: string) => a.trim() === foodName))
        ) {
          return { match: best, simScore: Math.max(simScore, 0.3) };
        }
      }
    } catch {
      // 匹配失败不阻断流程
