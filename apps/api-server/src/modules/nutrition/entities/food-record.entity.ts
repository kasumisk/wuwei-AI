...
export class FoodRecord {
  ...
  @Column({ type: 'enum', enum: RecordSource, default: RecordSource.SCREENSHOT, comment: '记录来源' })
  source: RecordSource;
  ...
  @Column({ type: 'int', default: 0, name: 'total_calories', comment: '总热量 kcal' })
  totalCalories: number;

  @Column({ type: 'enum', enum: MealType, default: MealType.LUNCH, name: 'meal_type', comment: '餐食类型' })
  mealType: MealType;

  @Column({ type: 'text', nullable: true, comment: 'AI 饮食建议' })
  advice?: string;

  @Column({ type: 'boolean', nullable: true, name: 'is_healthy', comment: '是否健康' })
  isHealthy?: boolean;
  ...
  @Column({ type: 'timestamp', name: 'recorded_at', default: () => 'CURRENT_TIMESTAMP', comment: '记录时间' })
  recordedAt: Date;
