...
export class UserProfileController {
  ...
  async skipOnboardingStep(
    ...
    const result = await this.userProfileService.skipOnboardingStep(user.id, step);
  ...
  async updateDeclaredProfile(
    ...
    const profile = await this.userProfileService.updateDeclaredProfile(user.id, dto);
    return {
      success: true,
      code: HttpStatus.OK,
      message: '更新成功',
  ...
  async getCompletionSuggestions(@CurrentAppUser() user: any): Promise<ApiResponse> {
    const result = await this.userProfileService.getCompletionSuggestions(user.id);
    return {
      success: true,
      code: HttpStatus.OK,
      message: '获取成功',
  ...
  async refreshInference(@CurrentAppUser() user: any): Promise<ApiResponse> {
    const inferred = await this.profileInferenceService.refreshInference(user.id);
    return {
      success: true,
      code: HttpStatus.OK,
      message: '推断已更新',
  ...
  async getGoalTransitionSuggestion(@CurrentAppUser() user: any): Promise<ApiResponse> {
    const suggestion = await this.profileInferenceService.getGoalTransitionSuggestion(user.id);
    return {
      success: true,
      code: HttpStatus.OK,
      message: '获取成功',
  ...
  async getCollectionTriggers(@CurrentAppUser() user: any): Promise<ApiResponse> {
    const reminders = await this.collectionTriggerService.checkCollectionTriggers(user.id);
    return {
      success: true,
      code: HttpStatus.OK,
      message: '获取成功',
