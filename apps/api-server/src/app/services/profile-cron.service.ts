...
export class ProfileCronService {
    ...
    try {
      ...
      for (const behavior of behaviors) {
        try {
          ...
          this.logger.warn(`用户 ${behavior.userId} 每日更新失败: ${(err as Error).message}`);
        }
      }
      ...
      this.logger.log(`每日画像更新完成: ${updated}/${behaviors.length} 用户, 耗时 ${elapsed}s`);
    } catch (err) {
      this.logger.error(`每日画像更新失败: ${(err as Error).message}`, (err as Error).stack);
    }
  }
   ...
   */
  private async updateDailyBehavior(behavior: UserBehaviorProfile): Promise<void> {
    ...
    for (const record of records) {
      ...
      dailyCalories[day] = (dailyCalories[day] || 0) + (record.totalCalories || 0);
    }

    const totalDays = Object.keys(dailyCalories).length;
    const healthyDays = Object.values(dailyCalories).filter(cal => cal <= dailyGoal).length;
    behavior.avgComplianceRate = totalDays > 0
      ? Number((healthyDays / totalDays).toFixed(2))
      : 0;
    ...
    try {
      ...
      for (const inf of inferred) {
        try {
          ...
          this.logger.warn(`用户 ${inf.userId} 分段更新失败: ${(err as Error).message}`);
        }
      }
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`每周分段更新完成: ${updated}/${inferred.length} 用户, 耗时 ${elapsed}s`);
    } catch (err) {
      this.logger.error(`每周分段更新失败: ${(err as Error).message}`, (err as Error).stack);
    }
  }

  private async updateWeeklySegmentation(inferred: UserInferredProfile): Promise<void> {
    ...
    if (goal === 'muscle_gain') {
      ...
      inferred.userSegment = goal === 'fat_loss' ? 'disciplined_loser' : 'active_maintainer';
    } else if (complianceRate < 0.4 && behavior && behavior.totalRecords >= 14) {
    ...
    const daysSinceLastRecord = lastRecordDate
      ? Math.floor((Date.now() - lastRecordDate.getTime()) / (1000 * 60 * 60 * 24))
      : 999;
    ...
    if (weeklyRecords.length >= 5) {
      const gaps: string[] = [];
      const avgProtein = weeklyRecords.reduce((s, r) => s + (Number(r.totalProtein) || 0), 0) / weeklyRecords.length;
      const avgCalories = weeklyRecords.reduce((s, r) => s + (Number(r.totalCalories) || 0), 0) / weeklyRecords.length;
    ...
    try {
      ...
      for (const inf of inferred) {
        try {
          ...
          this.logger.warn(`用户 ${inf.userId} 口味向量更新失败: ${(err as Error).message}`);
        }
      }
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`口味偏好更新完成: ${updated}/${inferred.length} 用户, 耗时 ${elapsed}s`);
    } catch (err) {
      this.logger.error(`口味偏好更新失败: ${(err as Error).message}`, (err as Error).stack);
    }
  }

  private async updateTastePrefVector(inferred: UserInferredProfile): Promise<void> {
    const userId = inferred.userId;
    ...
    for (const fb of feedbacks) {
      const daysSince = Math.floor((now - (fb.createdAt as Date).getTime()) / (1000 * 60 * 60 * 24));
