...
import { UserProfile, GoalType, GoalSpeed } from '../../entities/user-profile.entity';
...
export class ProfileInferenceService {
  ...
  async refreshInference(userId: string): Promise<UserInferredProfile | null> {
    ...
    if (profile.targetWeightKg && profile.weightKg) {
      const startWeight = inferred.goalProgress?.startWeight || Number(profile.weightKg);
      ...
      const progressPercent = totalDelta > 0 ? Math.round((currentDelta / totalDelta) * 1000) / 10 : 0;
      ...
      inferred.goalProgress = {
        ...
        trend: progressPercent >= 100 ? 'ahead' : progressPercent > 0 ? 'on_track' : 'behind',
      };
    }
   ...
   * 获取目标迁移建议
   */
  async getGoalTransitionSuggestion(userId: string): Promise<GoalTransitionSuggestion | null> {
    const profile = await this.profileRepo.findOne({ where: { userId } });
    if (!profile) return null;
  ...
  private inferUserSegment(
    ...
    if (complianceRate >= 0.7) {
      return goal === GoalType.FAT_LOSS ? 'disciplined_loser' : 'active_maintainer';
    }
