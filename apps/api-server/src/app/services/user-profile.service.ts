...
const CRITICAL_FIELDS = [
  'goal', 'goalSpeed', 'weightKg', 'allergens',
...
export class UserProfileService {
  ...
  async saveOnboardingStep(
    userId: string,
    step: number,
    dto: OnboardingStep1Dto | OnboardingStep2Dto | OnboardingStep3Dto | OnboardingStep4Dto,
  ): Promise<{
    profile: UserProfile;
  ...
  async getCompletionSuggestions(userId: string): Promise<{
    ...
    const fieldMeta: Record<string, { priority: 'high' | 'medium' | 'low'; reason: string; impact: string }> = {
      heightCm: { priority: 'high', reason: '计算基础代谢率的核心参数', impact: '推荐准确度提升 ~20%' },
      weightKg: { priority: 'high', reason: '计算基础代谢率的核心参数', impact: '推荐准确度提升 ~20%' },
      allergens: { priority: 'high', reason: '确保推荐食物安全，排除过敏原', impact: '安全性保障' },
      goal: { priority: 'high', reason: '决定营养分配策略', impact: '推荐准确度提升 ~15%' },
      dietaryRestrictions: { priority: 'medium', reason: '过滤不适合的食物', impact: '推荐准确度提升 ~10%' },
      healthConditions: { priority: 'medium', reason: '针对健康状况调整推荐', impact: '安全性 + 准确度提升' },
      discipline: { priority: 'medium', reason: '调整推荐约束松紧度', impact: '执行率提升 ~15%' },
      bingeTriggers: { priority: 'medium', reason: '暴食预防干预', impact: '坚持率提升 ~10%' },
      exerciseProfile: { priority: 'low', reason: '运动用户热量需求不同', impact: '热量计算更精确' },
      cookingSkillLevel: { priority: 'low', reason: '匹配烹饪难度', impact: '推荐可行性提升' },
    };
  ...
  calculateDailyGoal(profile: UserProfile): number {
    ...
    return this.calculateRecommendedCalories(tdee, profile.goal, profile.goalSpeed);
  }
  ...
  private async createSnapshotIfNeeded(
    ...
    const triggerType = changed.includes('goal')
      ...
      : changed.includes('weightKg')
        ? 'weight_update'
        : changed.includes('allergens') || changed.includes('dietaryRestrictions')
          ? 'restriction_change'
          : 'field_update';
    ...
    this.logger.log(`用户 ${userId} 关键字段变更快照已创建: [${changed.join(', ')}]`);
  }

  /**
   * 同步更新推断数据（BMR/TDEE/macroTargets）
   */
  private async syncInferredProfile(profile: UserProfile): Promise<UserInferredProfile | null> {
    if (!profile.gender || !profile.birthYear || !profile.heightCm || !profile.weightKg) {
      return null;
    }

    let inferred = await this.inferredRepo.findOne({ where: { userId: profile.userId } });
    if (!inferred) {
    ...
    const bmr = this.calculateBMR(profile);
    const tdee = this.calculateTDEE(bmr, profile.activityLevel);
    const recommendedCalories = this.calculateRecommendedCalories(tdee, profile.goal, profile.goalSpeed);
    ...
    inferred.macroTargets = this.calculateMacroTargets(recommendedCalories, profile.goal);
  ...
  private calculateMacroTargets(
    ...
    const ratios: Record<string, [number, number, number]> = {
      [GoalType.FAT_LOSS]: [0.35, 0.40, 0.25],
      [GoalType.MUSCLE_GAIN]: [0.40, 0.40, 0.20],
      [GoalType.HEALTH]: [0.25, 0.50, 0.25],
      [GoalType.HABIT]: [0.25, 0.50, 0.25],
    };
