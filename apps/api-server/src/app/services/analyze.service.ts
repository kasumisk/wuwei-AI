...
/**
 * AI 食物图片分析服务
 * 直接调用 OpenAI API（GPT-4o Vision）进行多模态分析
 */
@Injectable()
export class AnalyzeService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.model =
      this.configService.get<string>('VISION_MODEL') || 'gpt-4o';

    if (!this.openaiApiKey) {
      this.logger.warn(
        'OPENAI_API_KEY 未配置，AI 分析功能将不可用',
      );
    }
  }
  ...
  async analyzeImage(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(`OpenAI API 错误: ${response.status} ${err}`);
