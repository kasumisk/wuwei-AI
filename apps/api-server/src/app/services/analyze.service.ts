...
import { NutritionScoreService, NutritionScoreBreakdown } from './nutrition-score.service';
...
function resolveDecision(
  ...
  const decisions: Array<'SAFE' | 'OK' | 'LIMIT' | 'AVOID'> = ['SAFE', 'OK', 'LIMIT', 'AVOID'];
...
const GOAL_CONTEXT: Record<string, { label: string; focus: string }> = {
  ...
  muscle_gain: { label: '增肌', focus: '优先关注：蛋白质是否充足 + 热量不能太低' },
  health: { label: '均衡健康', focus: '优先关注：食物质量和营养均衡' },
  habit: { label: '改善饮食习惯', focus: '优先关注：食物质量和饱腹感，鼓励坚持记录' },
};

// AI 容错：未返回营养数据时的粗估
function estimateNutrition(totalCalories: number, category?: string): {
  protein: number; fat: number; carbs: number; quality: number; satiety: number;
} {
  const CATEGORY_DEFAULTS: Record<string, { quality: number; satiety: number }> = {
  ...
  const defaults = CATEGORY_DEFAULTS[category || ''] || { quality: 5, satiety: 5 };
  return {
...
export class AnalyzeService {
   ...
   */
  private async buildUserContext(userId?: string): Promise<{ context: string; goalType: string; profile: any }> {
...
- 当前时段：${mealHint}`;
      ...
      if (profile) {
        ...
        if (profile.foodPreferences?.length) ctx += `\n- 饮食偏好：${profile.foodPreferences.join('、')}`;
        if (profile.dietaryRestrictions?.length) ctx += `\n- 忌口：${profile.dietaryRestrictions.join('、')}`;
      }
  ...
  async analyzeImage(
    ...
    const { context: userContext, goalType, profile } = await this.buildUserContext(userId);
    ...
    try {
      ...
      if (userId && profile) {
        try {
          ...
          // 获取稳定性数据
          let stabilityData: { streakDays: number; avgMealsPerDay: number; targetMeals: number } | undefined;
          try {
            const behaviorProfile = await this.behaviorService.getProfile(userId);
            if (behaviorProfile) {
              stabilityData = {
                streakDays: behaviorProfile.streakDays || 0,
                avgMealsPerDay: behaviorProfile.totalRecords > 0 ? behaviorProfile.totalRecords / Math.max(1, behaviorProfile.streakDays || 1) : 3,
  ...
  private parseAnalysisResult(content: string): AnalysisResult {
    try {
      ...
      let totalProtein = typeof parsed.totalProtein === 'number' ? parsed.totalProtein : 0;
      let totalFat = typeof parsed.totalFat === 'number' ? parsed.totalFat : 0;
      let totalCarbs = typeof parsed.totalCarbs === 'number' ? parsed.totalCarbs : 0;
      let avgQuality = typeof parsed.avgQuality === 'number' ? parsed.avgQuality : 0;
      let avgSatiety = typeof parsed.avgSatiety === 'number' ? parsed.avgSatiety : 0;
      const totalCalories = typeof parsed.totalCalories === 'number' ? parsed.totalCalories : 0;

      // AI 容错：如果没返回汇总营养，从 foods 或粗估
      if (totalProtein === 0 && totalFat === 0 && totalCarbs === 0) {
        if (foods.some((f: any) => f.protein > 0)) {
          totalProtein = foods.reduce((s: number, f: any) => s + (f.protein || 0), 0);
          totalFat = foods.reduce((s: number, f: any) => s + (f.fat || 0), 0);
          totalCarbs = foods.reduce((s: number, f: any) => s + (f.carbs || 0), 0);
        } else {
      ...
      if (avgQuality === 0 || avgSatiety === 0) {
        if (foods.some((f: any) => f.quality > 0)) {
          avgQuality = avgQuality || Math.round(foods.reduce((s: number, f: any) => s + (f.quality || 5), 0) / Math.max(1, foods.length) * 10) / 10;
          avgSatiety = avgSatiety || Math.round(foods.reduce((s: number, f: any) => s + (f.satiety || 5), 0) / Math.max(1, foods.length) * 10) / 10;
        } else {
