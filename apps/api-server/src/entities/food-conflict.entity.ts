...
export class FoodConflict {
  ...
  @Column({ type: 'varchar', length: 50, comment: '冲突字段: calories / protein / category ...' })
  field: string;

  @Column({ type: 'jsonb', comment: '冲突来源: [{"source":"usda","value":165},{"source":"off","value":170}]' })
  sources: Array<{ source: string; value: any }>;

  @Column({ type: 'varchar', length: 20, nullable: true, comment: 'pending / auto_highest_priority / manual / averaged' })
  resolution?: string;

  @Column({ type: 'text', nullable: true, name: 'resolved_value', comment: '最终采用的值' })
