...
export class FoodLibraryManagementService {
  ...
  async findAll(query: GetFoodLibraryQueryDto) {
    const { page = 1, pageSize = 20, keyword, category, isVerified, primarySource, status } = query;
    const qb = this.foodLibraryRepo.createQueryBuilder('f');

    if (keyword) {
      qb.andWhere('(f.name ILIKE :kw OR f.aliases ILIKE :kw OR f.code ILIKE :kw)', { kw: `%${keyword}%` });
    }
    if (category) {
  ...
  async create(dto: CreateFoodLibraryDto): Promise<FoodLibrary> {
    ...
    const codeExisting = await this.foodLibraryRepo.findOne({ where: { code: dto.code } });
    if (codeExisting) {
  ...
  async update(id: string, dto: UpdateFoodLibraryDto, operator = 'admin'): Promise<FoodLibrary> {
    ...
    if (Object.keys(changes).length > 0) {
      await this.createChangeLog(id, saved.dataVersion, 'update', changes, undefined, operator);
    }
    return saved;
  }
  ...
  async batchImport(foods: CreateFoodLibraryDto[]): Promise<{ imported: number; skipped: number; errors: string[] }> {
    ...
    for (const dto of foods) {
      try {
        const existing = await this.foodLibraryRepo.findOne({ where: { code: dto.code } });
        if (existing) {
          skipped++;
          continue;
        }
  ...
  async toggleVerified(id: string, operator = 'admin'): Promise<FoodLibrary> {
    ...
    await this.createChangeLog(id, saved.dataVersion, 'verify', {
      isVerified: { old: !food.isVerified, new: food.isVerified },
    }, undefined, operator);
    return saved;
  }

  async updateStatus(id: string, newStatus: string, operator = 'admin'): Promise<FoodLibrary> {
    const food = await this.findOne(id);
    ...
    await this.createChangeLog(id, saved.dataVersion, newStatus === 'archived' ? 'archive' : 'update', {
  ...
  async getStatistics() {
    ...
    const conflictCount = await this.conflictRepo.count({ where: { resolution: 'pending' } });

    return { total, verified, unverified, byCategory, bySource, byStatus, pendingConflicts: conflictCount };
  }
  ...
  async getTranslations(foodId: string) {
    return this.translationRepo.find({ where: { foodId }, order: { locale: 'ASC' } });
  }

  async createTranslation(foodId: string, dto: CreateFoodTranslationDto) {
    await this.findOne(foodId); // validate food exists
    const existing = await this.translationRepo.findOne({ where: { foodId, locale: dto.locale } });
    if (existing) {
  ...
  async updateTranslation(translationId: string, dto: UpdateFoodTranslationDto) {
  ...
  async getSources(foodId: string) {
    return this.sourceRepo.find({ where: { foodId }, order: { priority: 'DESC' } });
  }
  ...
  private async createChangeLog(
    ...
    const log = this.changeLogRepo.create({ foodId, version, action, changes, reason, operator });
    return this.changeLogRepo.save(log);
  }

  // ==================== 冲突管理 ====================

  async getConflicts(query: { foodId?: string; resolution?: string; page?: number; pageSize?: number }) {
    const { foodId, resolution, page = 1, pageSize = 20 } = query;
    const qb = this.conflictRepo.createQueryBuilder('c')
  ...
  async resolveConflict(conflictId: string, dto: ResolveFoodConflictDto, operator = 'admin') {
