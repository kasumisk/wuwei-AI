...
export class FoodLibraryManagementController {
  ...
  async getConflicts(@Query() query: { foodId?: string; resolution?: string; page?: number; pageSize?: number }): Promise<ApiResponse> {
  ...
  async updateStatus(@Param('id') id: string, @Body('status') status: string): Promise<ApiResponse> {
  ...
  async createTranslation(@Param('id') id: string, @Body() dto: CreateFoodTranslationDto): Promise<ApiResponse> {
  ...
  async updateTranslation(@Param('translationId') translationId: string, @Body() dto: UpdateFoodTranslationDto): Promise<ApiResponse> {
    const data = await this.foodLibraryService.updateTranslation(translationId, dto);
    return { success: true, code: HttpStatus.OK, message: '更新成功', data };
  }

  @Delete('translations/:translationId')
  @ApiOperation({ summary: '删除食物翻译' })
  async deleteTranslation(@Param('translationId') translationId: string): Promise<ApiResponse> {
  ...
  async createSource(@Param('id') id: string, @Body() dto: CreateFoodSourceDto): Promise<ApiResponse> {
  ...
  async deleteSource(@Param('sourceId') sourceId: string): Promise<ApiResponse> {
  ...
  async resolveConflict(@Param('conflictId') conflictId: string, @Body() dto: ResolveFoodConflictDto): Promise<ApiResponse> {
