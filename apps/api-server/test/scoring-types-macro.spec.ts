...
describe('scoring.types — P0-3/P1-2 新增导出', () => {
  ...
  describe('getProteinPerKg / PROTEIN_PER_KG_BY_GOAL（P1-2 统一蛋白质公式）', () => {
    it('四个目标的系数应为 fat_loss=2.0 / muscle_gain=2.2 / health=1.3 / habit=1.1', () => {
      expect(PROTEIN_PER_KG_BY_GOAL.fat_loss).toBe(2.0);
      expect(PROTEIN_PER_KG_BY_GOAL.muscle_gain).toBe(2.2);
      expect(PROTEIN_PER_KG_BY_GOAL.health).toBe(1.3);
      expect(PROTEIN_PER_KG_BY_GOAL.habit).toBe(1.1);
    });

    it('getProteinPerKg 对未知 goal 应回退到 health', () => {
      expect(getProteinPerKg('unknown_goal')).toBe(1.3);
      expect(getProteinPerKg(null)).toBe(1.3);
      expect(getProteinPerKg(undefined)).toBe(1.3);
    });

    it('分析文档案例体重 72kg × fat_loss 应得到 144g 蛋白质（之前 1.2 g/kg 只得到 86g，偏差 −37%）', () => {
      expect(Math.round(72 * getProteinPerKg('fat_loss'))).toBe(144);
    });
  });
