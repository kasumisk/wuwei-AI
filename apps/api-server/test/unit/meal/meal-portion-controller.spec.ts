...
describe('MealPortionController', () => {
  ...
  describe('trimExcessFoods — 控制每餐食物数量', () => {
    it('早餐超过4个食物时移除低优先级', () => {
      ...
      const { kept, removed } = controller.trimExcessFoods(
        picks,
        'breakfast',
        policies,
      );
  ...
  describe('ensureRoleCoverage — 角色覆盖', () => {
    it('缺少主食时补位', () => {
      const picks = [
        ...
        makeScoredFood(
          makeFood({ id: 'v', name: '西兰花', category: 'vegetable' }),
        ),
      ];
      const candidates = [
        makeScoredFood(
      ...
      const policies = resolver.resolveAll([
        ...picks.map((p) => p.food),
        ...candidates.map((c) => c.food),
      ]);
      const usedNames = new Set<string>(picks.map((p) => p.food.name));

      const result = controller.ensureRoleCoverage(
        picks,
        candidates,
        policies,
        usedNames,
