 ...
 */
import { NutritionTargetService } from '../../../src/modules/diet/app/recommendation/pipeline/nutrition-target.service';
...
describe('NutritionTargetService', () => {
  ...
  describe('calculate — goal-based protein', () => {
    // P-ε 调参后真值：health=1.6, fat_loss=2.1, muscle_gain=2.4, habit=2.0 (g/kg)
    it('should calculate health protein at 1.6 g/kg', () => {
      const result = service.calculate({
        goal: 'health',
        weightKg: 75,
      });
      expect(result.protein).toBe(120); // 75 * 1.6 = 120
    });

    it('should calculate fat_loss protein at 2.1 g/kg', () => {
      const result = service.calculate({
        goal: 'fat_loss',
        weightKg: 80,
      });
      expect(result.protein).toBe(168); // 80 * 2.1 = 168
    });

    it('should calculate muscle_gain protein at 2.4 g/kg', () => {
      const result = service.calculate({
        goal: 'muscle_gain',
        weightKg: 70,
      });
      expect(result.protein).toBe(168); // 70 * 2.4 = 168
    });

    it('should enforce minimum 50g protein (FDA DV)', () => {
      const result = service.calculate({
        goal: 'health',
        weightKg: 40, // 40 * 1.6 = 64 → above 50 floor
      });
      expect(result.protein).toBe(64);
    });
  });
