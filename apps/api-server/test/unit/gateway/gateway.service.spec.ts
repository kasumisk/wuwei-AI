...
describe('GatewayService', () => {
  ...
  describe('recordUsage', () => {
    ...
    it('应该成功记录使用情况', async () => {
      ...
      mockPrisma.usageRecords.create.mockResolvedValue(createdRecord);

      const result = await service.recordUsage(mockUsageData);

      expect(result).toEqual(createdRecord);
      expect(mockPrisma.usageRecords.create).toHaveBeenCalledWith({
        data: {
          clientId: 'client-123',
          requestId: 'req-456',
          capabilityType: 'TEXT_GENERATION',
    ...
    it('应该处理保存错误', async () => {
      mockPrisma.usageRecords.create.mockRejectedValue(
