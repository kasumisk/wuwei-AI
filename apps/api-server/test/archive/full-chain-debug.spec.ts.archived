...
const F = {
  ...
  englishShellfish: [
    buildFood({
      name: 'shellfish shrimp fried rice',
      calories: 420,
      protein: 22,
      fat: 14,
      carbs: 55,
  ...
  chinesePeanut: [
    buildFood({ name: '花生米', calories: 200, protein: 8, fat: 14, carbs: 8 }),
  ],
  englishPeanut: [
    buildFood({
...
describe('【B1】过敏原中英文匹配', () => {
  it('B1-基准: 英文allergen + 含同义关键词英文食物名 → 应触发', () => {
    const result = checkAllergenConflict(
      F.englishShellfish,
      buildCtx({ allergens: ['shellfish'] }),
      LOCALE,
    );
  ...
  it('B1-2★: shellfish过敏 + 中文虾仁炒饭 → 期望avoid但实际可能miss', () => {
    ...
    expect(typeof result?.triggered === 'boolean' || result === null).toBe(
      true,
    );
  });
  ...
  it('B1-4★: shellfish过敏整体决策 - 吃虾仁应被avoid', () => {
    ...
    expect(['avoid', 'caution', 'recommend']).toContain(
...
describe('【B2】low_sodium饮食限制', () => {
  it('B2-1★: low_sodium限制 + 高钠食物 → checkRestrictionConflict应触发', () => {
    const result = checkRestrictionConflict(
      F.highSodiumFood,
      CTX.health,
      LOCALE,
    );
    const totalSodium = F.highSodiumFood.reduce(
      (s, f) => s + (f.sodium ?? 0),
      0,
    );
    console.log(
      `[B2-1] low_sodium + ${totalSodium}mg钠: triggered=${result?.triggered}`,
    );
    if (!result?.triggered) {
      reportBug(
        'B2-1',
...
describe('【B3】diabetes_type2子串匹配', () => {
  it('B3-1★: healthConditions=[diabetes_type2] 是否被识别为糖尿病', () => {
    ...
    console.log(
      `[B3-1] diabetes_type2 精确匹配=${exactMatch}, 子串匹配=${substrMatch}`,
    );
...
describe('【F1】正常功能验证', () => {
  it('F1-1: 高血压 + 5000mg钠 → healthConditionRisk触发', () => {
    ...
    const results = checkHealthConditionRisk(
      F.highSodiumFood,
      CTX.health,
      LOCALE,
      th,
    ...
    console.log(
      `[F1-1] 高血压+5000mg: ${results.length}个risk, triggered=${results[0]?.triggered}`,
    );
  ...
  it('F1-5: 改善习惯夜宵高热量 → 降级', () => {
    ...
    console.log(
      `[F1-5] 夜宵${total}kcal, 剩余${CTX.habit.remainingCalories}kcal → ${decision.recommendation}`,
    );
    expect(['caution', 'avoid']).toContain(decision.recommendation);
  });
  ...
  it('F1-6: 动态阈值 - 减脂过预算容忍 ≤ 增肌', () => {
    ...
    console.log(
      `[F1-6] overBudgetMargin: 减脂=${fatLossTh.overBudgetMargin} vs 增肌=${muscleGainTh.overBudgetMargin}`,
    );
    expect(fatLossTh.overBudgetMargin).toBeLessThanOrEqual(
      muscleGainTh.overBudgetMargin,
    );
  });
  ...
  it('F1-7: 动态阈值 - 所有场景字段完整', () => {
    ...
    for (const [name, ctx] of Object.entries(CTX)) {
      ...
      console.log(
        `[F1-7] ${name}: overBudget=${th.overBudgetMargin}, lowProt=${th.lowProteinMeal}g`,
      );
    }
  });
});
