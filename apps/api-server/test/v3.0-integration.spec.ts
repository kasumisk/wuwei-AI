...
describe('V3.0 Phase1 — SignalTrace ordering', () => {
  const {
    getSignalPriority,
  } = require('./modules/decision/config/signal-priority.config');
  ...
  test('health_constraint should always be first', () => {
    const trace = buildSignalTrace(
      ['under_target', 'protein_gap', 'health_constraint'],
      'fat_loss',
    );
  ...
  test('trace is sorted by priority descending', () => {
    const trace = buildSignalTrace(
      ['under_target', 'over_limit', 'protein_gap'],
      'muscle_gain',
    );
...
describe('V3.0 Phase2 — DecisionToneResolver', () => {
  ...
  test('health_constraint focus overrides to urgent', () => {
    const r = resolver.resolve({
      goalType: 'health',
      verdict: 'recommend',
      coachFocus: 'health_constraint 优先',
    });
  ...
  test('over_limit focus overrides to urgent', () => {
    const r = resolver.resolve({
      goalType: 'fat_loss',
      verdict: 'recommend',
      coachFocus: 'over_limit 今日',
    });
    expect(r.toneKey).toBe('urgent');
  });

  test('resolveModifier returns non-empty string', () => {
    const m = resolver.resolveModifier({
      goalType: 'health',
      verdict: 'caution',
    });
    ...
    const alts = [
      {
        name: 'A',
        score: 0.7,
        comparison: { caloriesDiff: 50, proteinDiff: 0 },
      },
      {
        name: 'B',
        score: 0.7,
        comparison: { caloriesDiff: -150, proteinDiff: 0 },
      },
    ];
    ...
    const alts = [
      {
        name: 'A',
        score: 0.6,
        comparison: { caloriesDiff: 0, proteinDiff: 0 },
      },
      {
        name: 'B',
        score: 0.6,
        comparison: { caloriesDiff: 0, proteinDiff: 20 },
      },
    ];
    const ranked = attachRankScores(alts, ctx);
    expect(ranked[0].name).toBe('B');
    ...
    const alts = [
      {
        name: 'X',
        score: 0.9,
        comparison: { caloriesDiff: -300, proteinDiff: 25 },
      },
    ];
    const ranked = attachRankScores(alts, ctx);
